        ...
        dispatcher.register(Commands.literal("ms").requires(s -> s.hasPermission(2))
            .then(Commands.literal("round")
                .then(Commands.literal("on").executes(ctx -> {
                    ...
                    ctx.getSource().sendSuccess(() -> Component.literal("§a[MineStrike] Мод включен!"), true);
                    return 1;
                }))
                .then(Commands.literal("off").executes(ctx -> {
                    MatchSystem.isModEnabled = false;
                    if (MatchSystem.isMatchStarted) {
                        RoundManager.forceStopMatch();
                    }
                    ctx.getSource().sendSuccess(() -> Component.literal("§c[MineStrike] Мод выключен! Матч принудительно завершен."), true);
                    return 1;
                }))
                // Управление самим матчем
                .then(Commands.literal("startmatch").executes(ctx -> {
                    if (!MatchSystem.isModEnabled) { ctx.getSource().sendFailure(Component.literal("§cОшибка: Мод выключен! Напишите /ms round on")); return 0; }
                ...
                // Команды внутри матча (требуют startmatch)
                .then(Commands.literal("start").executes(ctx -> {
                    if (!MatchSystem.isMatchStarted) { ctx.getSource().sendFailure(Component.literal("§cОшибка: Матч не запущен! Напишите /ms round startmatch")); return 0; }
                ...
                .then(Commands.literal("stop").executes(ctx -> {
                    ...
                    RoundManager.broadcastMessage("§c[MineStrike] Раунд остановлен и сброшен. Игроки вернулись на спавн.");
                    return 1;
                }))
                ...
                .then(Commands.literal("reset").executes(ctx -> {
                    ...
                    MatchSystem.isRoundActive = false;
                    StatsManager.resetStats();
                    RoundManager.broadcastMessage("§e[MineStrike] Матч сброшен и находится в ожидании начала раунда.");
                    return 1;
                }))
                // Настройки лимитов
            ...
            .then(Commands.literal("commandaddplayers")
                .then(Commands.literal("red").then(Commands.argument("player", EntityArgument.player()).executes(ctx -> { ServerPlayer p = EntityArgument.getPlayer(ctx, "player"); MatchSystem.removeFromAllTeams(p.getUUID()); MatchSystem.T_TEAM.add(p.getUUID()); ctx.getSource().sendSuccess(() -> Component.literal("§c" + p.getGameProfile().getName() + " §7добавлен за §cКРАСНЫХ (T)"), true); return 1; })))
                .then(Commands.literal("blue").then(Commands.argument("player", EntityArgument.player()).executes(ctx -> { ServerPlayer p = EntityArgument.getPlayer(ctx, "player"); MatchSystem.removeFromAllTeams(p.getUUID()); MatchSystem.CT_TEAM.add(p.getUUID()); ctx.getSource().sendSuccess(() -> Component.literal("§b" + p.getGameProfile().getName() + " §7добавлен за §bСИНИХ (CT)"), true); return 1; })))
                ...
                .then(Commands.literal("info").executes(ctx -> {
                    ...
                    MatchSystem.T_TEAM.forEach(uuid -> sb.append("§7- §f").append(uuid.toString()).append("\n")); // Имена подставятся сервером
                    sb.append("§b§lCOUNTER-TERRORISTS (CT):\n");
                    MatchSystem.CT_TEAM.forEach(uuid -> sb.append("§7- §f").append(uuid.toString()).append("\n"));
